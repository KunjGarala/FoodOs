...
public class RestaurantTableController {
    ...
    @Operation(
            ...
            description = "Creates a new table for a restaurant with specified capacity and floor position"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "201",
    ...
    @Operation(
            ...
            description = "Updates table configuration (capacity, position, section) but not status"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    @Operation(
            ...
            description = "Changes table lifecycle status (VACANT → OCCUPIED → BILLED → DIRTY → VACANT)"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Table status updated successfully",
                    content = @Content(schema = @Schema(implementation = TableStatusResponseDto.class))
            ),
            @ApiResponse(responseCode = "404", description = "Table not found"),
    ...
    @Operation(
            ...
            description = "Fetches single table with live status and current order information"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    @Operation(
            ...
            description = "Fetches all tables across all restaurants with optional status filter"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    public ResponseEntity<Page<TableResponseDto>> getAllTables(
            ...
            @Parameter(description = "Filter by status", schema = @Schema(allowableValues = {"VACANT", "OCCUPIED", "BILLED", "DIRTY", "RESERVED"}))
     ...
     */
    @Operation(
            summary = "Get tables by restaurant (floor plan)",
            description = "Fetches all tables for a specific restaurant with position coordinates for POS floor plan view"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Tables fetched successfully",
                    content = @Content(schema = @Schema(implementation = TableFloorPlanDto.class))
            ),
            @ApiResponse(responseCode = "404", description = "Restaurant not found"),
            @ApiResponse(responseCode = "403", description = "Access denied")
    })
     ...
     */
    @Operation(
            summary = "Get tables by restaurant chain",
            description = "Fetches table summaries for all outlets in a restaurant chain (multi-outlet dashboard)"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Chain table summaries fetched successfully",
                    content = @Content(schema = @Schema(implementation = RestaurantChainTablesSummaryDto.class))
            ),
    ...
    @Operation(
            ...
            description = "Soft deletes a table (marks as inactive). Cannot delete if table is OCCUPIED."
    )
    @ApiResponses({
    ...
    @Operation(
            ...
            description = "Combines multiple tables for large party seating. All tables must be VACANT."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Tables merged successfully",
                    content = @Content(schema = @Schema(implementation = MergeTablesResponseDto.class))
            ),
     ...
     */
    @Operation(
            summary = "Transfer table order",
            description = "Moves guests and their order from one table to another. Source must be OCCUPIED, destination must be VACANT."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Table transfer successful",
                    content = @Content(schema = @Schema(implementation = TransferTableResponseDto.class))
            ),
    ...
    @Operation(
            ...
            description = "Fetches table utilization analytics including occupancy rate, average turn time, and most used tables"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
                    description = "Analytics fetched successfully",
                    content = @Content(schema = @Schema(implementation = TableAnalyticsDto.class))
            ),
            @ApiResponse(responseCode = "404", description = "Restaurant not found"),
            @ApiResponse(responseCode = "403", description = "Access denied")
    })
